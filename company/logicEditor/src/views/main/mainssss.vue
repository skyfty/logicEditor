    ...
    export default{
        data(){
            return{
                ...
                planArr: [
                    { x: -300, y: 300, name: '第一块区域' },
                    { x: 300, y: 300, name: '第二块区域'},
                    { x: -300, y: -300, name: '第三块区域' },
                    { x: 300, y: -300, name: '第四块区域' }
                ],
                events: {
        ...
        methods: {
            ...
            initCamera(){
                ...
                // 计算正交相机的参数  
                const aspectRatio = window.innerWidth / window.innerHeight; // 横纵比  
                const left = -window.innerWidth / aspectRatio / 2;  
                const right = window.innerWidth / aspectRatio / 2;  
                const top = window.innerHeight / 2;  
                const bottom = -window.innerHeight / 2;  
                
                // 创建正交相机  
                this.camera = new THREE.OrthographicCamera(left, right, top, bottom, cP.near, cP.far);  
            ...
            createCube(){
                for(let i = 0; i < this.planArr.length; i++){
                    ...
                    const cube = new THREE.Mesh(this.geometry(100, 100, 0.), material);
                    this.scene.add(cube);
                    // 添加名称
            ...
            pickEvents(normalizedPosition, scene, camera, callback){
                // 如果存在拾取的对象，则恢复颜色
                if(this.events.pickedObject && this.events.pickedObject.material && this.events.pickedObject.material.emissive){
