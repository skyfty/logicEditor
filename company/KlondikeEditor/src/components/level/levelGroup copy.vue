<template>
  <div class="nav">
    ...
    <el-table :data="tableData.slice((currentPage-1)*sizeChange,currentPage*sizeChange)"
      ...
      <el-table-column :prop="item.prop" :label="item.label" v-for="(item,index) in tableLable" :key="index">
        <template slot-scope="scope" v-if="item.prop == 'state'">
  ...
  data() {
    return {
      ...
      tableLable: [
        ...
        {label:"牌局ID", prop:'deckGame'},
        {label:"金币", prop:'gold'},
        {label:"经验", prop:'experience'},
        {label:"钻石", prop:'diamond'},
        {label:"状态", prop:'state'},
      ],
      formInline: [{
        amount: '',
  ...
  computed: { 
    levelAmount() {
      ...
      for (let i = 0; i < this.formInline[0].amount;i++) {
        this.tableData.push({id: i+1,difficultyLevel:'',deckGame:'',gold:'',experience:'',diamond:'',state:false})
      }
    },
  },
